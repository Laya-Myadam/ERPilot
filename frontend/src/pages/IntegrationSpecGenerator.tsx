...
export default function IntegrationSpecGenerator() {
  ...
  const loadSample = () => { setIntegrationName("Oracle Cloud HCM to ADP Benefits Integration"); setSourceSystem("Oracle Cloud HCM"); setTargetSystem("ADP Workforce Now"); setDirection("Source → Target (One-way)"); setFrequency("Daily (batch)"); setDataObjects("Employee personal data (new hires, terminations, name/address changes), Benefits enrollment elections, Dependent information, Life event triggers (marriage, birth, divorce)"); setBusinessPurpose("Automatically sync Oracle Cloud HCM employee data to ADP for benefits administration. When an employee is hired, terminated, or has a qualifying life event in Oracle, ADP must be updated within 24 hours to ensure correct benefits enrollment and payroll deductions."); setVolume("~850 employees total. Daily changes: 2-5 new hires, 1-3 terminations, 10-20 personal data changes. Open enrollment period: up to 800 transactions in 2-week window."); setSpecialReqs("Must handle ADP's proprietary file format (834 EDI for benefits). SSN must be masked in transit logs. Failed records must be quarantined and emailed to HR Operations within 1 hour. Must support retroactive effective dates for life events."); };
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="card-glass rounded-xl p-6 space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={generate} disabled={loading || !businessPurpose.trim()} className="flex-1 py-2.5 rounded-lg bg-accent-cyan text-black font-semibold text-sm hover:opacity-90 disabled:opacity-40 transition">
